...
all: clean travis build fmt lint test staticcheck vet install ## Runs a clean, build, fmt, lint, test, staticcheck, vet and install.
...
travis: prebuild ## Executes the necessary steps to configure travis
	...
	for osarch in "${osarches[@]}"; do
		osarch=${osarch/\//-};
		files+='\n          - cross/'"${NAME}-${osarch}"'\n';
		files+='          - cross/'"${NAME}-${osarch}"'.md5\n';
		files+='          - cross/'"${NAME}-${osarch}"'.sha256';
	done;

	# Remove all the file lines from the travis file.
	sed -i '\_.*- cross/'"${NAME}"'-*._d' "$travis_file";
